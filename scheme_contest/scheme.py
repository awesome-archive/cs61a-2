...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try: 
            ...
            raise SchemeError('{0}: incorrect number of arguments'.format(e))
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        lambda_func = LambdaProcedure(target.second, expressions.second, env)
...
def do_define_macro(expressions, env):
    ...
    if isinstance(expressions.first, Pair) and scheme_symbolp(expressions.first.first):
        env.bindings[expressions.first.first] = MacroProcedure(expressions.first.second, expressions.second, env)
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        """ When eval_all returns scheme_eval with true as 3rd param
        it changes optimized_eval to True"""
        ...
        if tail and not scheme_symbolp(expr) and not self_evaluating(expr):
            # if expr is in a tail context return Thunk instance
            return Thunk(expr, env)
        else:
            result = Thunk(expr, env)
        # BEGIN
        if scheme_atomp(expr): # Expr is atomic 
