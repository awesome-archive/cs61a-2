...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while next_place != hive:
            if self.min_range <= dist <= self.max_range and next_place.bees:
...
# BEGIN Problem 8
class WallAnt(Ant):
    """A WallAnt that has 4 armor and does nothing"""
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        while next_place != None:
            if next_place.ant:
                if next_place.ant.container and next_place.ant.ant and next_place.ant.ant not in self.is_doubled:
                    next_place.ant.ant.damage = next_place.ant.ant.damage * 2
                    self.is_doubled += [next_place.ant.ant]
                if next_place.ant not in self.is_doubled:
                    next_place.ant.damage = next_place.ant.damage * 2
                    self.is_doubled += [next_place.ant]
